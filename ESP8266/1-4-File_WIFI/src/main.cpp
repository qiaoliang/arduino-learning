...
/*
   ...
   2000微秒--------135度；
   2500微秒--------180度；
   高电平脉冲每g≈1度≈(2500-500)/180,所以1微秒≈0.1度  不用太较真..
...
#define BLINKER_WITHOUT_SSL // ESP8266 堆栈不足要采用非SSL加密方式连网通信
...
{                // 脉宽高电平 500微秒 到 2500微秒 之间，对应舵机0°～180°可转角度
  ...
  if (I >= 0 && I < ss)
  {
    ...
    if (ss == 4)
      Servo4(I); // 四轴机械臂 限制Y轴与Z轴最大钝角与最小锐角,与装配角度有关
    return false;
  }
...
{ //-----四轴机械臂 限制Y轴与Z轴最大钝角与最小锐角,与舵机装配角度有关---
  ...
  { // 控制Y角度
    if (v < 3350)
    { // Y+Z 锐角小于3350脉宽,限值3350;约 30度
      newdms[2] += 3350 - v;
      if (newdms[2] > maxdms[2])
        newdms[1] += newdms[2] - maxdms[2];
    }
    if (v > 4550) // Y+Z 钝角大于4550脉宽,限值4550;约140度
      newdms[2] -= v - 4550;
    if (mindms[2] > newdms[2])
      newdms[1] -= mindms[2] - newdms[2];
  }
  if (I == 2)
  {               // 控制Z角度
    if (v < 3350) // Y+Z 锐角小于3350脉宽,限值3350;
      newdms[1] += 3350 - v;
    if (newdms[1] > maxdms[1])
      newdms[2] += newdms[1] - maxdms[1];
    if (v > 4550) // Y+Z 钝角大于4550脉宽,限值4550;
      newdms[1] -= v - 4550;
    if (mindms[1] > newdms[1])
      newdms[2] -= mindms[1] - newdms[1];
  }
}
...
{                                             // Blinker.attachData(dataRead) {"X":109}
...
void heartbeat()
{
  ...
  if (ss > 4)
    Slider5.print(map(newdms[4], 500, 2500, 0, 180));
  if (ss > 5)
    Slider6.print(map(newdms[5], 500, 2500, 0, 180));
}
// 物连网 点灯科技APP 滑动进度条会执行下列事件
void sliderX(int32_t value) { dataRead("X" + String(value)); }
void sliderY(int32_t value) { dataRead("Y" + String(value)); }
...
void setup()
{
  ...
  if (ss != 4 && ss != 5)
    ss = 6; // ss 决定程序控制是几轴机械臂
  if (ss == 4 && todms((float)rawdms[2]) < 2000)
    rawdms[2] = 2490; // 四轴Z舵机默认在 180度 位置
  for (int I = 0; I < ss; I++)
  {
  ...
  if (WiFi.status() == WL_CONNECTED && 6 < BlinKey.length())
  {
    ...
    if (ss > 4)
      Slider5.attach(sliderB); // 附加 滑动条  事件
    if (ss > 5)
      Slider6.attach(sliderT); // 附加 滑动条  事件
  }
}
//-----------------------------------------------------------------------
//
...
String Command(String t)
{
  ...
  { // 用 "F Auto.txt" 改变动作文件名
    if (S == "")
      S = "/Auto.txt"; // 用 "F test.txt" 指定动作文件名
    if (!S.startsWith("/"))
      S = "/" + S; // 比较字符串前缀
    if (1 > S.indexOf(".", 1))
      S = S + ".txt";                    // 比较字符串后缀
  ...
  if (t.equalsIgnoreCase("delay"))
  {                              // 动作脚本中 delay xx 由这段代码执行
  ...
  if (t.equalsIgnoreCase("SH"))
  {                                      // 保存XYZBTE舵机当前位置到 H.txt 文件
  ...
  { // HOME 让XYZBTE六个电机到指定位置
    if (SPIFFS.exists("/H.txt"))
    { // 如果有 H.txt 则执行
      Command("R H.txt");
      return output();
    }
  ...
  { // 保存XYZETB电机新角度指令到自动执行文件
    ret = "";
    if (S.toInt() > 0)
    {                                     // S xx 转为 delay xx 保存到文件
      ret = "delay " + String(S.toInt()); // 保存delay xx到文件
    }
    else if (S != "")
    {          // S delay xx 保存 delay xx  到文件
  ...
  { // C Auto;C test;清空指定文件内容并删除文件
    if (S == "")
      S = file; // 参数若未指令文件，删除file
    if (!S.startsWith("/"))
      S = "/" + S; // 比较字符串前缀
    if (1 > S.indexOf(".", 1))
      S = S + ".txt"; // 比较字符串后缀
  ...
  if (t.equalsIgnoreCase("step"))
  { // 改变舵机速度,step 0=原速,step 1=减速;
    Step = 0 != S.toInt();
    return "Step:" + String(Step); //
  }
  if (t.equalsIgnoreCase("format"))
  {                    // 格式化 开发板闪存文件系统 清除所有文件,需要重新上传
    SPIFFS.format();   // 格式化
    FSInfo info;       // 信息
    SPIFFS.info(info); // 取闪存文件系统信息
  ...
  { // UP and down  你想写啥就写啥,别问我
    Servo180(-1, 0);
    ret = "UP..";
    return ret;
  }
  if (t.equalsIgnoreCase("DIR"))
  {                    // 枚举文件并输出
    FSInfo info;       // 信息
    SPIFFS.info(info); // 取闪存文件系统信息
    ret = "闪存.已用:" + String(info.usedBytes) + "字节;可用:" + String(info.totalBytes) + "字节;\n";
  ...
  {                // R   运行file变量指定文件的动作指令
    ...
    if (v != 0 && S == String(v))
    {              // R Auto   执行Auto.txt 1次
      Autorun = v; // R test   执行test.txt 1次
      return ret;  // 若 R 参数为整数,设置 Autorun 变量循环执行 Auto.txt 次数
    }
    if (S == "")
      S = file; // 参数若未指定文件,则执行file变量里的文件
    Auto = S;
  }
  else
    Auto = t; // 对于不可识别的指令尝试以动作文件执行,如 "Q" 变会成 "/Q.txt" 若存在则执行
  if (!Auto.startsWith("/"))
    Auto = "/" + Auto; // 比较字符串前缀
  if (1 > Auto.indexOf(".", 1))
    Auto = Auto + ".txt"; // 比较字符串后缀
  if (SPIFFS.exists(Auto))
  { // 判断动作文件是否存在.若有此文件则执行
    ret = "R " + Auto + "\n";
    // Serial.println(ret);
    if (Response && Auto != "/H.txt")
    {                                   // R指令判断是否有待应答网页请求
